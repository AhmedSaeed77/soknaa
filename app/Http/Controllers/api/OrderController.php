...
سوف يقوم فريق سٌكنة بالتواصل مع الطرف الاخر والرد عليك ومن ثم تحديد موعد للرؤية الشرعية عبر زووم';
    ...
    public function getAllOrdersSuccess()
    {
        try
        {
            ...
            $users = User::where('reason','توفيقي من التطبيق والحمد لله')->orderBy('created_at', 'desc')->get();
            $orders_data = OrdersSuccessResource::collection($orders);
            $users_data = OrdersSuccessResource::collection($users);
            $data = $orders_data->merge($users_data);
